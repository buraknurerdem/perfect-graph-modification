...
OUTPUTS_DIR="outputs_comp/cp_$timestamp"
CONSOLE_LOG_DIR="console_logs_comp/cp_$timestamp"
summary_file="outputs_comp/cp_summary_$timestamp.csv"
...
for input_file in "$INPUTS_DIR"/*; do
    for param_file in "$PARAMS_DIR"/*; do
        ...
        ./output_exe_completion "$param_file" "$input_file" "$output_file" "$summary_file" > "$console_log_file"
    done
done